...
use crate::{IRCodeGen, Context};
use mun_syntax::ast::{NameOwner, AstNode, TypeAscriptionOwner, StmtKind};
...
pub(crate) fn emit(fun: &ast::FunctionDef, context: &mut Context) {
    ...
    let arguments:Vec<BasicTypeEnum> = fun.param_list()
        .expect("missing param list")
        .params()
        .map(|arg| {
            let ty = arg.ascribed_type().expect("missing argument type");
            super::types::known_type(ty, context).expect("unknown type").into()
        })
        .collect();

    let fn_type = match fun.ascribed_type() {
        Some(t) => super::types::known_type(t, context).expect("Unknown type").fn_type(arguments.as_slice(), false),
    ...
    let body = context.module.get_context().append_basic_block(&fn_ref, "body");
    context.builder.position_at_end(&body);

    let mut symbol_table:HashMap<String, BasicValueEnum> = HashMap::new();
    for (idx, arg) in fun.param_list()
        .expect("missing param list")
        .params().enumerate() {
        let name = arg.name().expect("missing arg name").syntax().text().to_string();
    ...
    if fun.ascribed_type().is_some() {
        let ret_expr = emit_expr(fun.body().expect("missing body").expr().expect("missing return expression"),
            context, &mut symbol_table);

        context.builder.build_return(Some(&ret_expr.expect("no return value")) );
    }

    context.functions.insert(name, fn_ref);
}

pub(crate) fn emit_expr_stmt(stmt: &ast::ExprStmt, context: &mut Context, symbols: &mut HashMap<String, BasicValueEnum>) -> Option<BasicValueEnum> {
    emit_expr(stmt.expr()?, context, symbols )
}

pub(crate) fn emit_expr(expr: &ast::Expr, context: &mut Context, symbols: &mut HashMap<String, BasicValueEnum>) -> Option<BasicValueEnum> {
    match expr.kind() {
        ast::ExprKind::NameRef(name) => {
            symbols.get(&name.syntax().text().to_string()).map(|v| *v)

        },
        ast::ExprKind::Literal(lit) => {
            match lit.syntax().first_token().unwrap().kind() {
                SyntaxKind::FLOAT_NUMBER|SyntaxKind::INT_NUMBER => {
                    let value = lit.syntax().text().to_string().parse::<f64>().ok()?;
                    Some(context.module.get_context().f32_type().const_float(value).into())
                },
                _ => unreachable!()
            }
        },
