...
use inkwell::values::{AnyValueEnum, BasicValueEnum, InstructionOpcode, StructValue};
use inkwell::{
...
pub(crate) fn module_ir_query(db: &impl IrDatabase, file_id: FileId) -> Module {
    ...
    for def in definitions {
        match def {
            ModuleDef::Function(f) => {
                let fun = gen_function_signature(db, *f, &module);
    ...
    for (f, value) in function_values.iter_mut() {
        gen_function_body(db, *f, *value, &module);
...
pub(crate) fn gen_function_signature(
    db: &impl IrDatabase,
    f: Function,
    module: &Module,
) -> FunctionValue {
...
pub(crate) fn gen_function_body(
    db: &impl IrDatabase,
    f: Function,
    fun: FunctionValue,
    module: &Module,
